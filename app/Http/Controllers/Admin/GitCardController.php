...
public function store(Request $request)
{
    ...
    if ($request->hasFile('image')) {
        ...
        $image->move(public_path('images/gitcards'), $imageName);
        $data['image'] = 'images/gitcards/'.$imageName;
    }
...
public function update(Request $request, GitCard $gitcard)
{
    ...
    if ($request->remove_image) {
        if ($gitcard->image && file_exists(public_path($gitcard->image))) {
            unlink(public_path($gitcard->image));
            $data['image'] = null;
        }
    }
